...
def edm_sampler_partial_denoise(
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
        ...
        print(f'desired var: {(kappa_i ** 2).cpu().numpy()}; actual var: {torch.linalg.norm(D_out_plus_kappa_i_noise - x_0).cpu().numpy() ** 2 / (64 * 64 * 3)}')
...
@click.option('--S_churn', 'S_churn',      help='Stochasticity strength', metavar='FLOAT',                          type=click.FloatRange(min=0), default=40., show_default=True)
