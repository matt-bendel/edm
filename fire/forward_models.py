...
def get_operator(deg, device):
    ...
    if deg not in accepted_operators:
    ...
    if deg == 'inp_box':
        ...
        H = Inpainting(3, 64, missing, mask, device)
        ...
        H = SRConv(kernel / kernel.sum(), 3, 64, device, stride=factor)
    elif deg == 'blur_uni':
        H = Deblurring(torch.Tensor([1 / 9] * 9).to(device), 3, 64, device)
    elif deg == 'blur_gauss':
        ...
        coloring = True
        H = Colorization(64, device)
    elif deg[:2] == 'sr':
        sr = True
        blur_by = int(deg[2:])
        H = SuperResolution(3, 64, blur_by, device)
    else:
        H = Denoising(3, 64, device)

    # compute s_max
    with torch.no_grad():
        b_k = torch.randn(1, 3, 64, 64).to(device)
        for _ in range(50):
